...
class GradCache:
    ...
    def build_cache(self, *reps: Tensor, **loss_kwargs) -> [List[Tensor], Tensor]:
        ...
        """
        _reps = reps
        reps = [r.detach().requires_grad_() for r in reps[:2]]
        reps += _reps[2:]
        with autocast() if self.fp16 else nullcontext():
    ...
    def cache_step(
        ...
        model_inputs, scores = (model_inputs[0], model_inputs[1]), model_inputs[2]
        ...
        for idx,(model, x) in enumerate(zip(self.models, model_inputs)):
            model_reps, rnd_states = self.forward_no_grad(model, x)
            all_reps.append(model_reps)
            all_rnd_states.append(rnd_states)

        all_reps.append(torch.tensor(scores["score0"], device=all_reps[0].device))
        all_reps.append(torch.tensor(scores["score1"], device=all_reps[1].device))
