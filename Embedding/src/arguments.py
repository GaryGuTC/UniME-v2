...
class ModelArguments:
    ...
    rerank_embedding: str = field(
        default = "embedding",
        metadata = {"help": "judge model whether for embedding or rerank: due to the qwen2.5VL model different"}
    )
...
class DataArguments:
    ...
    random_shuffle: bool = field(
        default=False, metadata={"help": "random shuffle training dataset"}
    )
    custom_data_path: str = field(
        default=None, metadata={"help": "custom data path"}
    )
    rerank_output_path: str = field(
        default=None, metadata={"help": "rerank output path"}
    )
    select_hard_negative_num: int = field(
        default=8, metadata={"help": "rerank output path"}
    )
    hard_negaitve_data_path: str = field(
        default="", metadata={"help": "huggingface dataset name"}
    )
...
class TrainingArguments(TrainingArguments):
    ...
    loss_type: str = field(
        default="MMEB", metadata={"help": "loss type"}) # DPO
    
    loss_type_KL_soft: str = field(
        default = "KL",
        metadata = {"help": "judge model whether for embedding or rerank: due to the qwen2.5VL model different"}
    )
    select_hard_negative_num_4_training: int = field(
        default=8, metadata={"help": "rerank output path"}
    )
