...
from src.model_utils import LLAVA_NEXT, QWEN2_VL, PHI3V, process_vlm_inputs_fns, process_vlm_rerank_inputs_fns
...
class EvalCollator_rerank:
    ...
    def __call__(self, examples):
        ...
        inputs = process_vlm_rerank_inputs_fns[self.model_args.model_backbone](examples,
                                        processor = self.processor,
                                        max_length = self.data_args.max_len,
                                         pairwise_listwise="pairwise")
        return inputs

@dataclass
class hardnegativeCollatorContrastive:
    data_args: DataArguments
    model_args: ModelArguments
    processor: ProcessorMixin
    ...
    def _get_batch_inputs(self, examples, text_keyname, image_keyname):
        texts, images = [], []
        examples_news = []
        for each in examples: examples_news += each
        for example in examples_news: # examples
            # @ruimeng filter invalid data examples here will lead to fail to sync across devices (unequal batch size)
            # use dummy input for now
            if example is None or not example:
                text, image = '  ', None
...
class UniME_v2_data_collector:
    ...
    def __call__(self, examples):
        ...
        pos_inputs = self._get_batch_inputs(examples, "pos_text", "pos_image")
        neg_inputs = self._get_batch_inputs(examples, "neg_text", "neg_image")
        scores_inputs = self._get_batch_scores(examples, "score0", "score1")
        return qry_inputs, pos_inputs, scores_inputs
    ...
    def _get_batch_scores(self, examples, text_keyname, image_keyname):
        texts, images = [], []
        examples_news = []
        for each in examples: examples_news += each
        for example in examples_news: # examples
            ...
            texts.append(text)
            images.append(image)
        inputs = {'score0': texts, 'score1': images}
        return inputs

@dataclass
class EvalCollator_rerank_listwise:
    data_args: DataArguments
    model_args: ModelArguments
    processor: ProcessorMixin
