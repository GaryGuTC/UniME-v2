...
from transformers import PreTrainedModel, AutoModelForCausalLM, AutoConfig, LlavaOnevisionForConditionalGeneration

from peft import LoraConfig, get_peft_model, PeftModel

from src.arguments import ModelArguments, TrainingArguments
from src.model_utils import LLAVA_NEXT, QWEN2_VL, PHI3V, LLAVA_OV, QWEN2_5_VL, backbone2model, get_backbone_name, print_master
...
class MMEBModel(nn.Module):
    ...
    def __init__(self,
                 ...
                 rerank_embedding: str = "embedding",
        ...
        self.rerank_embedding = rerank_embedding
    ...
    def build(cls, model_args: ModelArguments, training_args: TrainingArguments=None, **kwargs):
        ...
        if model_backbone == PHI3V:
            ...
            if model_args.rerank_embedding == "embedding":
                config._attn_implementation = "flash_attention_2"
                config.padding_side = "left"
                config.use_cache = False
        ...
        if model_args.lora:
            ...
            model = cls(
                ...
                rerank_embedding=model_args.rerank_embedding,
    ...
    def embedding_forward(self, qry, tgt):
    ...
    def rerank_forward(self): #! @need edit
        return None

    def forward(self, qry: Dict[str, Tensor] = None, tgt: Dict[str, Tensor] = None, *args, **kwargs):
        if self.rerank_embedding == "embedding": return self.embedding_forward(qry, tgt)
        else: return self.rerank_forward()
