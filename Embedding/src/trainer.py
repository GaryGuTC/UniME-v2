...
from src.loss import  SoftLabelSoftKLContrastiveLoss, DistributedSoftLabelSoftKLContrastiveLoss
...
class MMEBTrainer(Trainer):
    def __init__(self, *args, **kwargs):
        ...
        self._signature_columns = ['query_text', 'query_image', 'target_text', 'target_image', 'label_ids', 'label', "score0", "score1"]
...
class GradCacheTrainer(MMEBTrainer):
    """
...
class GradCacheLateProcessTrainer(MMEBTrainer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.is_ddp = dist.is_initialized()
        self._dist_loss_scale_factor = dist.get_world_size() if self.is_ddp else 1
        loss_fn_cls = DistributedSoftLabelSoftKLContrastiveLoss if self.is_ddp else SoftLabelSoftKLContrastiveLoss
        loss_fn = loss_fn_cls(temperature=self.model.temperature, select_hard_negative_num=self.args.select_hard_negative_num_4_training)
        ...
        from grad_cache.grad_cache_UniME_v2 import GradCache
        self.gc = GradCache(
            models=[self.model, self.model],
            chunk_sizes=[self.args.gc_q_chunk_size, self.args.gc_p_chunk_size],
            loss_fn=loss_fn,
    ...
    def training_step(self, model, inputs, *args, **kwargs) -> torch.Tensor:
        model.train()
        queries, targets, scores = inputs
        queries, targets = {'qry': queries}, {'tgt': targets}

        _distributed = self.args.local_rank > -1
        self.gc.models = [model, model]
        loss = self.gc(queries, targets, scores, no_sync_except_last=_distributed)
        return loss / self._dist_loss_scale_factor
