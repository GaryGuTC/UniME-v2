...
class MMEBRerankDataCollator(BaseDataCollator):
    @property
...
class MMEBRerankDataCollator_4_llavaOV(BaseDataCollator):
    @property
    def PAD_TOKEN_ID(self) -> int:
        return self.tokenizer.pad_token_id
    ...
    def __call__(self, messages: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        rerank_inputs = self.processor.apply_chat_template(
            rerank_messages,
        ...
        return dict(
            ...
            attention_mask=rerank_attention_mask,
            pixel_values=rerank_pixel_values,
            image_sizes=rerank_inputs["image_sizes"], 
            image_grid_thw=rerank_image_grid_thw,
            labels=rerank_labels,
        )